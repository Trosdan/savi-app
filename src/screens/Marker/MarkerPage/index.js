    ...
    >
        <View style={{ backgroundColor: '#f88e1e', height: hp('30%') }}>
            <View style={{ backgroundColor: '#f07e1e', height: hp('7%'), flexDirection: 'row', alignItems: 'center' }}>
                ...
                />
                <Text style={{ fontSize: RFPercentage(2.5), marginBottom: hp('.45%'), alignContent: 'center', textAlignVertical: 'center', color: '#fff'}}>
                    Cruz Roja Colombiana
                </Text>
            </View>
        </View>
        <Surface style={{ elevation: 10 }}>
            <View style={{ height: hp("10%"), backgroundColor: '#242f3e', flexDirection: 'row', alignItems: 'center', justifyContent: 'center'}}>
                <Button onPress={() => {scrollRef.getNode().scrollTo({x: 0}), console.log(ScrollPos)}} style={{ width: wp("50%")}} mode="text">
                    <Text style={{ color: '#fff', fontSize: RFPercentage(2.5) }}>Informações</Text>
                </Button>
                <Button onPress={() => {scrollRef.getNode().scrollTo({x: SCREEN_WIDTH}), console.log(ScrollPos)}} style={{ width: wp("50%")}} mode="text">
                    <Text style={{ color: '#fff', fontSize: RFPercentage(2.5) }}>Avaliações</Text>
                </Button>
                <Image
                        source={require("../../../assets/images/MarkerMenuBar.png")}
                        //resizeMode="contain"
                        //resizeMethod="auto"
                        style={{ height: hp("1%"), width: wp("50%"), position: 'absolute', bottom: 0, left: wp("0%")}}
                />
            </View>
        </Surface>
        <Animated.ScrollView 
